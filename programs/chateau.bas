...
80 IF A(RO,7)<>0 OR A(RO,9)<>0 OR A(RO,10)<>0 THEN GOSUB 6660:REM OBJECTS
90 IF A(RO,7)>98 THEN PRINT "One of the doors is locked":PRINT "preventing you from exploring":PRINT "further"
100 IF A(RO,8)<>0 THEN PRINT TAB(3);"LOOK OUT!":PRINT "THERE IS A ";M$(A(RO,8));" HERE!":IF RND(1)>.7 AND A(RO,8)<>1 THEN PRINT "THE ";M$(A(RO,8));" ATTACKS!":KW=1:GOSUB 1400:GOTO 40
...
1430 R=RND(1)
1440 IF R<.5 THEN PRINT "There is nothing to fight here"
1450 IF R>=.5 THEN PRINT "You can't fight empty air!"
1460 RETURN
1470 G$=M$(A(RO,8))
...
1840 FOR J=1 TO 5
1850 IF P(J)<>0 AND T(J)<>0 THEN PRINT J;" - ";O$(P(J))
1860 NEXT J
1870 INPUT "Enter the number to choose";J
1880 IF P(J)=0 OR T(J)=0 THEN PRINT "YOU DO NOT HAVE THE ";O$(P(J)):GOTO 1870
1890 PRINT "RIGHT, SO YOU CHOOSE TO FIGHT":PRINT "WITH THE ";O$(P(J))
1900 FF=INT(FF*2/J)
...
2240 IF RND(1)>.25 AND HT>0 AND MT>0 THEN FOR T=1 TO 1600:NEXT T:GOTO 2130
...
2700 IF BOX=1 THEN IF RND(1)>.6 THEN PRINT TAB(4);"It holds nothing but dust...":RETURN
2710 IF RO=13 OR BOX=1 THEN PRINT "IT IS EMPTY!":RETURN
2720 IF BOX=0 THEN PRINT "INSIDE YOU FIND A PARCHMENT, WITH":PRINT "THE FOLLOWING MESSAGE: 'A little":PRINT "man can be bound by gold'":BOX=1
2730 RETURN
2740 REM ***********
...
3100 IF RND(1)>.5 THEN PRINT "HE WOULD ACCEPT ANYTHIHG":PRINT "THAT HE REALLY WANTS" :GOTO 3210
...
7840 DATA "QUARTERSTAFF",0,"MORNINGSTAR",0,"FALCHION",0
